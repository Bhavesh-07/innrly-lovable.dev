...
from fastapi import FastAPI, HTTPException, Request, Response, Depends, File, UploadFile
...
EMAIL_FROM = os.environ.get("EMAIL_FROM", "sales@innrly.com")
...
async def update_seo(payload: SeoSettingsPayload, current_user: dict = Depends(require_permission("seo"))):
    ...
    try:
        ...
        """
        priority_str = str(payload.priority) if payload.priority is not None else "0.8"
        values = (
            payload.page_path, payload.title, payload.description, payload.keywords,
            payload.og_title, payload.og_description, payload.og_image,
            1 if payload.in_sitemap else 0,
            payload.changefreq or "monthly",
            priority_str
        )
