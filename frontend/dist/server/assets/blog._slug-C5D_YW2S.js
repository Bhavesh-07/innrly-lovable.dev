...
var $$splitComponentImporter = () => import("./blog._slug-zOaCA9Ql.js");
var posts = {
...
var Route = createFileRoute("/blog/$slug")({
	loader: async ({ params }) => {
		...
		try {
			const { fetchSeoData } = await import("./seo-X3geolyH.js").then((n) => n.a);
	...
	head: ({ loaderData }) => ({
		...
		links: loaderData ? [{
			rel: "canonical",
			href: `https://www.innrly.com/blog/${loaderData.slug}`
		}] : [],
		scripts: loaderData ? [{
			...
			children: JSON.stringify({
				...
				mainEntityOfPage: {
					"@type": "WebPage",
					"@id": `https://www.innrly.com/blog/${loaderData.slug}`
				}
			})
		}, {
			type: "application/ld+json",
