...
export const Route = createFileRoute("/pricing")({
  ...
  head: ({ loaderData }) => ({
    meta: [
      ...getMetaTags(loaderData?.seo || null, defaultSeoData["/pricing"], "/pricing"),
      {
    ...
    scripts: [
      {
        ...
        children: JSON.stringify({
          ...
          offers: [
            ...
            { "@type": "Offer", name: "Professional", price: "299", priceCurrency: "USD" },
          ],
        }),
      },
      {
...
type TileColor = "teal" | "violet" | "emerald" | "amber" | "rose";
...
const TILES: Tile[] = [
  {
    ...
    blurb:
      "Real-time portfolio heartbeat. Daily snapshots, calendar view, KPI rollups across every property.",
    icon: Gauge,
    inPlan: "starter",
...
const COMPARE: Array<[string, boolean, boolean | string]> = [
  ["Pulse Dashboard", true, true],
  ["Early Bird daily snapshot", true, true],
  ["Calendar View", true, true],
  ["Night Audit+", true, true],
  ["OTA Reconciliation", true, true],
  ["Happy Guest scores", true, true],
  ["STR Report", true, true],
  ["Reporting library", true, true],
  ["Email support", true, true],
  ["Accounting integration", false, true],
  ["A/P Automation", false, true],
  ["A/R Validation", false, true],
  ["Labor Snapshot", false, true],
  ["Mobile app", false, true],
  ["Innrly Pay", false, "Annual only"],
...
function PricingPage() {
  ...
  const pricingTestimonials = allTestimonials.filter((t: Testimonial) => t.page === "pricing");
  ...
  return (
    <div className="bg-background">
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="relative mx-auto max-w-5xl px-4 pt-20 pb-10 text-center sm:px-6 lg:px-8 lg:pt-24">
          <h1 className="text-4xl font-bold text-foreground sm:text-5xl lg:text-6xl">
            Build your plan, <span className="text-gradient">not a column.</span>
          </h1>
          <p className="mt-4 text-lg text-muted-foreground max-w-2xl mx-auto">
            Toggle a tier. Watch capabilities light up across your stack. Add modules only when you
            need them.
          </p>
        </div>
      </section>
      ...
      {/* CONTROL DECK */}
      <Section id="plans" className="pt-4 pb-8 scroll-mt-20">
        <div className="mx-auto max-w-4xl rounded-2xl border border-border bg-card/60 backdrop-blur p-4">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-stretch lg:justify-between">
            <div className="grid grid-cols-2 gap-3 flex-1">
              {(["starter", "pro"] as const).map((p) => {
                ...
                const blurb =
                  p === "starter"
                    ? "Single property or small portfolios."
                    : "Growing portfolios that need automation.";
                ...
                return (
                  <button
                    ...
                    onClick={() => {
                      setPlan(p);
                      track("pricing_plan_toggle", { plan: p, billing });
                    }}
                  ...
                  >
                    <span
                      className={`absolute inset-x-0 top-0 h-[3px] transition-all ${
                        isActive
                          ? "bg-gradient-to-r from-accent via-cta to-accent shadow-[0_0_12px_var(--color-accent)] animate-pulse"
                          : "bg-border/40"
                      }`}
                      aria-hidden
                    />
                    {isPopular && !isActive && (
                      <span className="absolute right-2 top-2 rounded-full bg-accent/15 px-2 py-0.5 text-[9px] font-bold uppercase tracking-wider text-accent ring-1 ring-inset ring-accent/30">
                        Most popular
                      </span>
                    )}
                    {isActive && (
                      <span className="absolute right-2 top-2 rounded-full bg-accent px-2 py-0.5 text-[9px] font-black uppercase tracking-wider text-primary-foreground shadow">
                        {isPopular ? "Popular · Active" : "Active"}
                      </span>
                    )}
                    <div className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">
                      {name}
                    </div>
                    <div className="mt-0.5 text-[11px] text-muted-foreground/90 leading-tight">
                      {blurb}
                    </div>
                    ...
                    <div className="mt-1.5 text-[10px] font-medium leading-tight">
                      {billing === "yearly" ? (
                        <span className="text-emerald-300">
                          Billed ${yearlyTotal.toLocaleString()} once per year · setup waived
                        </span>
                      ) : (
                        <span className="text-muted-foreground">
                          + $299 one-time setup · cancel anytime
                        </span>
                      )}
                    </div>
                  </button>
            ...
            <div className="flex items-center justify-between gap-3 lg:flex-col lg:items-end lg:justify-center">
              <div className="flex flex-col items-end gap-1">
                <div className="flex items-center gap-1 rounded-full bg-surface/60 p-1 border-[1.5px] border-accent/60">
                  <button
                    type="button"
                    onClick={() => {
                      setBilling("monthly");
                      track("pricing_billing_toggle", { billing: "monthly", plan });
                    }}
                    aria-pressed={billing === "monthly"}
                    className={`min-h-9 rounded-full px-4 text-xs font-bold uppercase tracking-wider transition-all ${
                      billing === "monthly"
                        ? "bg-cta text-primary-foreground shadow-[0_0_14px_var(--color-cta)]"
                  ...
                  <button
                    ...
                    onClick={() => {
                      setBilling("yearly");
                      track("pricing_billing_toggle", { billing: "yearly", plan });
                    }}
                ...
                <span className="text-[10px] font-medium tracking-wide text-muted-foreground">
                  {billing === "yearly" ? (
                    <span className="text-emerald-300">2 months free · setup waived on annual</span>
                  ) : (
                    <>Save 25% + setup waived on annual</>
                  )}
                </span>
              </div>
              ...
              <div className="flex items-center gap-2 text-xs">
                ...
                <span className="text-muted-foreground">
                  <span className="font-bold text-foreground">{unlockedCount}</span> of{" "}
                  {TILES.length} lit
                </span>
              </div>
            </div>
      ...
      <Section className="pt-2 pb-16">
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 md:grid-cols-12 md:auto-rows-[120px]">
          {TILES.map((tile) => {
            ...
            return (
              <div
                key={tile.id}
                className={`group relative overflow-hidden rounded-2xl p-5 transition-all duration-500 ${tile.span} ${
                  unlocked
                    ? `border-2 ${c.border} bg-card ${c.glow} ring-1 ring-inset ${c.ring}`
                    : "border border-dashed border-border/50 bg-card/20 opacity-40 grayscale hover:opacity-80 hover:grayscale-0"
                }`}
              >
                {unlocked && (
                  <span
                ...
                {!unlocked && (
                  <div className="absolute right-3 top-3 flex items-center gap-1 rounded-full bg-muted px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider text-muted-foreground">
                ...
                {unlocked && (
                  <span className="absolute right-3 top-3 flex h-2 w-2" aria-hidden>
                    <span
                      className={`absolute inline-flex h-full w-full animate-ping rounded-full ${c.dotPing} opacity-75`}
                    />
                    <span
                      className={`relative inline-flex h-2 w-2 rounded-full ${c.dot} ${c.dotShadow}`}
                    />
                  </span>
                )}
                {tile.feature && unlocked && (
                  <div
                ...
                <div className="relative flex h-full flex-col">
                  <div
                    className={`mb-3 inline-flex h-10 w-10 items-center justify-center rounded-xl ring-1 ${
                      unlocked
                        ? `${c.iconBg} text-primary-foreground ${c.iconShadow}`
                  ...
                  {isUpgrade && plan === "starter" && (
                    ...
                    >
                      <span className="rounded-full bg-cta px-3 py-1.5 text-xs font-bold text-primary-foreground shadow-glow">
                        Unlock with Pro →
                      </span>
                    </button>
                  )}
      ...
      {/* ADD-ONS — placed above comparison so users see full stack first */}
      <Section className="py-12">
        ...
        <div className="mt-8 grid gap-5 md:grid-cols-2">
          {ADDONS.map((a) => {
            ...
            // Distinct neon accents per add-on
            const accent = isPay
              ? {
            ...
            return (
              <button
                ...
                onClick={() => {
                  if (!isShift) return;
                  setShift((s) => {
                    track("pricing_addon_toggle", { addon: "shift", added: !s, plan, billing });
                    return !s;
                  });
                }}
                className={`group relative overflow-hidden rounded-2xl border-2 bg-card p-6 text-left transition-all ${
                  active
                    ? `${accent.borderActive} ${accent.glowActive}`
                    : `${accent.border} ${accent.glow}`
                } ${!isShift ? "cursor-default" : ""}`}
              >
                <span
                ...
                <div className="relative mt-8 flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-baseline justify-between gap-3 flex-wrap">
                      ...
                      <span className={`text-base font-bold ${accent.priceText}`}>
                        {a.price}{" "}
                        <span className="text-xs font-normal text-muted-foreground">{a.unit}</span>
                      </span>
                    </div>
                    ...
                    {isShift && (
                      <div className="mt-4 inline-flex items-center gap-2 text-xs font-semibold">
                        ...
                        >
                          <span
                            className={`block h-3 w-3 rounded-full bg-card transition-transform ${shift ? "translate-x-3" : ""}`}
                          />
                        </span>
                        <span className={shift ? "text-orange-300" : "text-muted-foreground"}>
                          {shift ? "Added to your plan" : "Tap to add"}
                        </span>
                      </div>
                    )}
                    {a.highlight && (
                      <div
                        className={`mt-4 inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider transition-all ${
                          payActive
                            ? "bg-blue-400 text-white shadow-[0_0_12px_rgb(59_130_246/0.8)]"
      ...
      {/* COMPARISON TABLE */}
      <Section id="compare" className="py-12 scroll-mt-20">
        <SectionHeading
          ...
          <table className="w-full min-w-[640px] text-sm">
            <thead>
              <tr className="border-b border-border bg-surface/40 text-left">
                ...
                <th scope="col" className="px-5 py-4 text-center font-semibold text-foreground">
                  Starter
                </th>
                <th scope="col" className="px-5 py-4 text-center font-semibold text-foreground">
                  Professional
                  <span className="ml-2 inline-flex items-center rounded-full bg-accent/15 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider text-accent">
                    Popular
                  </span>
                </th>
              </tr>
            ...
            <tbody className="divide-y divide-border/60">
              {COMPARE.map(([label, s, p]) => (
                <tr key={String(label)} className="odd:bg-surface/20">
                  ...
                  <td className="px-5 py-3 text-center">
                    {typeof p === "string" ? (
                      <span className="text-xs font-medium text-accent">{p}</span>
                    ) : p ? (
                      <Check className="mx-auto h-4 w-4 text-accent" aria-label="Included" />
                    ) : (
                      <span className="text-muted-foreground" aria-label="Not included">
      ...
      <LogosStrip />
      {/* GLOBAL TESTIMONIALS */}
      <Testimonials testimonials={pricingTestimonials} />
      ...
      {/* STICKY CHECKOUT BAR — generic conversion nudge, no plan/price */}
      <div className="sticky bottom-3 z-40 mx-auto mb-3 max-w-5xl px-3">
        <div className="flex flex-col gap-3 rounded-2xl border border-accent/30 bg-card/95 p-3 pl-5 shadow-glow backdrop-blur sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-3 flex-wrap">
            <span className="relative flex h-2 w-2" aria-hidden>
              <span className="absolute inline-flex h-full w-full animate-ping rounded-full bg-accent opacity-75" />
              <span className="relative inline-flex h-2 w-2 rounded-full bg-accent" />
            </span>
            <span className="text-sm font-semibold text-foreground">90-day free trial</span>
            <span className="text-xs text-muted-foreground">No credit card · cancel anytime</span>
          </div>
          <Button asChild size="lg" className="bg-cta hover:opacity-90 whitespace-nowrap">
            <Link
              to="/contact"
              onClick={() => track("pricing_cta_click", { location: "sticky_bar", plan, billing })}
            >
              Start 90-day trial <ArrowRight className="ml-1 h-4 w-4" />
            </Link>
          </Button>
        </div>
      </div>
