...
export function getSeoForPath(pathname: string): SeoData {
  ...
  if (defaultSeoData[normalized]) {
    return defaultSeoData[normalized]
  }
  const matchedKey = Object.keys(defaultSeoData).find(
    (k) => k !== '/' && normalized.startsWith(k)
  )
  if (matchedKey) {
    return defaultSeoData[matchedKey]
  }
  return defaultSeoData['/']
}

export async function fetchSeoData(pathname: string, origin: string = ''): Promise<SeoData> {
  const normalized = pathname === '' ? '/' : pathname.replace(/\/+$/, '') || '/'
  try {
    ...
    if (res.ok) {
      ...
      if (data && data.title) {
        return {
          ...
          inSitemap: data.in_sitemap !== undefined ? Boolean(data.in_sitemap) : true,
...
export function getMetaTags(
  dynamicSeo?: Partial<SeoData> | Partial<SeoTags> | null,
  fallbackSeo?: Partial<SeoData>,
  pathname?: string
) {
  const def = pathname && defaultSeoData[pathname] ? defaultSeoData[pathname] : undefined
  ...
  tags.push({ property: 'og:site_name', content: 'Innrly' })
  tags.push({ name: 'twitter:card', content: 'summary_large_image' })
  if (ogTitle) {
    tags.push({ name: 'twitter:title', content: ogTitle })
  }
  if (ogDescription) {
    tags.push({ name: 'twitter:description', content: ogDescription })
  }
  if (ogImage) {
