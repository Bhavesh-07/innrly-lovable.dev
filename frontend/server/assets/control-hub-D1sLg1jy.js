...
import { Users, Laptop, UserCheck, Mail, Settings, Search, BookOpen, ListTree, Database, MessageSquareQuote, LogOut, Bell, ChevronDown } from "lucide-react";
import { W as Wordmark, g as getApiBase } from "./router-CBR-JcUy.js";
...
function AdminLayout() {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      const stored = localStorage.getItem("innrly_admin_profile");
      if (stored) {
        try {
          const parsed = JSON.parse(stored);
          if (parsed.name && parsed.role && parsed.avatar) {
            setProfile(parsed);
          }
        } catch (e) {
      ...
      const settingsUrl = `${getApiBase()}/settings`;
      fetch(settingsUrl).then((res) => res.json()).then((data) => {
        if (data && data.innrly_admin_profile) {
          try {
            const parsed = JSON.parse(data.innrly_admin_profile);
            if (parsed.name && parsed.role && parsed.avatar) {
              setProfile(parsed);
              localStorage.setItem("innrly_admin_profile", JSON.stringify(parsed));
            }
          } catch (e) {
            console.error("Failed to parse database admin profile:", e);
          }
        }
      }).catch((err) => console.error("Failed to load profile from database settings:", err));
    }
  }, []);
  useEffect(() => {
    if (isLoginPage) {
  ...
  return /* @__PURE__ */ jsxs("div", { className: `admin-portal flex h-screen font-sans antialiased overflow-hidden transition-colors duration-300 ${"bg-[#f8f9fa] text-slate-700"}`, children: [
    /* @__PURE__ */ jsxs("aside", { className: `w-64 border-r flex flex-col shrink-0 transition-colors duration-300 ${"bg-white border-slate-200"}`, children: [
      ...
      /* @__PURE__ */ jsxs("div", { className: `p-6 border-b flex items-center gap-3 shrink-0 transition-colors duration-300 ${"border-slate-100"}`, children: [
        /* @__PURE__ */ jsx("div", { className: "h-10 w-10 rounded-full overflow-hidden bg-slate-100 border border-slate-200", children: /* @__PURE__ */ jsx("img", { src: profile.avatar, alt: profile.name, className: "h-full w-full object-cover" }) }),
        /* @__PURE__ */ jsxs("div", { className: "min-w-0", children: [
          /* @__PURE__ */ jsx("span", { className: `text-sm font-bold block leading-tight truncate ${"text-slate-800"}`, children: profile.name }),
          /* @__PURE__ */ jsx("span", { className: "text-xs text-slate-400 block mt-0.5 font-medium", children: profile.role })
        ] })
      ] }),
      /* @__PURE__ */ jsxs("nav", { className: "flex-1 p-4 space-y-1.5 overflow-y-auto", children: [
        ...
        /* @__PURE__ */ jsxs(Link, { to: "/control-hub/testimonials", activeProps: {
          className: "bg-slate-100 text-indigo-600 font-semibold"
        }, inactiveProps: {
          className: "text-slate-600 hover:text-slate-900 hover:bg-slate-50"
        }, className: "flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm font-medium transition-all", children: [
          /* @__PURE__ */ jsx(MessageSquareQuote, { className: "h-4.5 w-4.5" }),
          "Testimonials"
        ] }),
        ...
        /* @__PURE__ */ jsxs("div", { className: "flex items-center gap-4", children: [
          ...
          /* @__PURE__ */ jsxs("div", { className: "relative", children: [
            /* @__PURE__ */ jsxs("div", { onClick: () => {
              ...
              /* @__PURE__ */ jsx("img", { src: profile.avatar, alt: profile.name, className: "h-8 w-8 rounded-full border border-slate-200 object-cover" }),
            ...
            showProfileMenu && /* @__PURE__ */ jsxs("div", { className: `absolute right-0 mt-2 w-48 border rounded-xl shadow-lg py-2 z-50 animate-in fade-in slide-in-from-top-2 duration-200 ${"bg-white border-slate-200 text-slate-700"}`, children: [
              /* @__PURE__ */ jsxs("div", { className: `px-4 py-2 border-b ${"border-slate-100"}`, children: [
                /* @__PURE__ */ jsx("span", { className: `font-bold text-xs block ${"text-slate-800"}`, children: profile.name }),
                /* @__PURE__ */ jsx("span", { className: "text-[10px] text-slate-400 block", children: profile.role })
              ] }),
    ...
    showProfileModal && /* @__PURE__ */ jsx("div", { className: "fixed inset-0 bg-black/50 flex items-center justify-center z-[100] animate-in fade-in duration-200", children: /* @__PURE__ */ jsxs("div", { className: `w-full max-w-md p-6 rounded-2xl shadow-xl animate-in zoom-in-95 duration-200 ${"bg-white text-slate-800 border border-slate-200"}`, children: [
      /* @__PURE__ */ jsx("h3", { className: "text-base font-bold mb-4", children: "Edit Profile Settings" }),
      /* @__PURE__ */ jsxs("div", { className: "space-y-4", children: [
        /* @__PURE__ */ jsxs("div", { className: "flex items-center gap-4", children: [
          /* @__PURE__ */ jsx("div", { className: "h-16 w-16 rounded-full overflow-hidden border border-slate-200 bg-slate-50 shrink-0", children: /* @__PURE__ */ jsx("img", { src: editAvatar || "https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&q=80&w=100&h=100", alt: "Preview", className: "h-full w-full object-cover", onError: (e) => {
            e.target.src = "https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&q=80&w=100&h=100";
          } }) }),
          /* @__PURE__ */ jsxs("div", { className: "flex-1 min-w-0", children: [
            /* @__PURE__ */ jsx("label", { className: "text-[10px] font-bold text-slate-400 uppercase tracking-wider block mb-1", children: "Avatar Image URL" }),
            /* @__PURE__ */ jsx("input", { type: "text", value: editAvatar, onChange: (e) => setEditAvatar(e.target.value), className: `w-full px-3 py-1.5 border rounded-lg text-xs focus:outline-none focus:border-indigo-500 ${"bg-slate-50 border-slate-200 text-slate-800"}`, placeholder: "https://images.unsplash.com/..." })
          ] })
        ] }),
        /* @__PURE__ */ jsxs("div", { children: [
          /* @__PURE__ */ jsx("label", { className: "text-[10px] font-bold text-slate-400 uppercase tracking-wider block mb-1", children: "Name" }),
          /* @__PURE__ */ jsx("input", { type: "text", value: editName, onChange: (e) => setEditName(e.target.value), className: `w-full px-3 py-1.5 border rounded-lg text-xs focus:outline-none focus:border-indigo-500 ${"bg-slate-50 border-slate-200 text-slate-800"}`, placeholder: "Name" })
        ] }),
        /* @__PURE__ */ jsxs("div", { children: [
          /* @__PURE__ */ jsx("label", { className: "text-[10px] font-bold text-slate-400 uppercase tracking-wider block mb-1", children: "Role / Job Title" }),
          /* @__PURE__ */ jsx("input", { type: "text", value: editRole, onChange: (e) => setEditRole(e.target.value), className: `w-full px-3 py-1.5 border rounded-lg text-xs focus:outline-none focus:border-indigo-500 ${"bg-slate-50 border-slate-200 text-slate-800"}`, placeholder: "Role" })
        ] })
      ] }),
      /* @__PURE__ */ jsxs("div", { className: "flex justify-end gap-2 mt-6", children: [
        /* @__PURE__ */ jsx("button", { onClick: () => setShowProfileModal(false), className: `px-4 py-2 text-xs font-semibold rounded-lg border transition-all cursor-pointer ${"border-slate-200 text-slate-500 hover:bg-slate-50 hover:text-slate-800"}`, children: "Cancel" }),
        /* @__PURE__ */ jsx("button", { onClick: async () => {
          ...
          const updated = {
            name: editName.trim(),
            role: editRole.trim(),
            avatar: editAvatar.trim() || "https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&q=80&w=100&h=100"
          };
          setProfile(updated);
          localStorage.setItem("innrly_admin_profile", JSON.stringify(updated));
          const settingsUrl = `${getApiBase()}/settings`;
          const token = localStorage.getItem("innrly_admin_token") || "";
          try {
            ...
            toast.error("Error saving to database, but stored locally!");
            setShowProfileModal(false);
          }
        }, className: "px-4 py-2 text-xs font-semibold rounded-lg bg-indigo-600 hover:bg-indigo-700 text-white transition-all cursor-pointer shadow-sm hover:shadow", children: "Save Changes" })
      ] })
    ] }) })
  ] });
}
